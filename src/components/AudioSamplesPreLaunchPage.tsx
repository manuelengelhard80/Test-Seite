...
export const AudioSamplesPreLaunchPage: React.FC<AudioSamplesPreLaunchPageProps> = ({ onBack }) => {
  ...
  const AUDIO_URLS = [
    "https://assets.anytourl.com/uploads/1dcb8360b74ce7620a4ba03440c56d9a.mp3", // Terminvereinbarung (Direkter HTTPS-Link)
    "", // Rezeptbestellung (placeholder - insert link here when available)
    ""  // Notfall-Triage (placeholder - insert link here when available)
  ];
  ...
  const togglePlay = (index: number) => {
    ...
    if (playingIndex === index) {
      ...
      const audioUrl = AUDIO_URLS[index];
      
      if (audioUrl) {
        const tryPlay = (url: string, fallbacks: string[]) => {
          console.log(`AudioSamplesPreLaunchPage attempting to play audio: ${url}`);
          ...
          const handleError = (e: any) => {
            console.warn(`AudioSamplesPreLaunchPage warning loading/playing audio from ${url}:`, e);
            cleanup();
            if (fallbacks.length > 0) {
              const nextUrl = fallbacks[0];
              const remainingFallbacks = fallbacks.slice(1);
              console.log(`AudioSamplesPreLaunchPage falling back to: ${nextUrl}`);
          ...
          audio.play().catch(err => {
            console.warn(`AudioSamplesPreLaunchPage play promise rejected for ${url}:`, err);
            if (audioRef.current === audio) {
              handleError(err);
            }
          });
        ...
        const fallbacks = index === 0 ? ["/termin.mp3?v=3", "/termin.wav?v=3"] : [];
        tryPlay(audioUrl, fallbacks);
      } else {
            ...
            {samples.map((sample, idx) => (
              <div key={idx} className="bg-slate-50 rounded-3xl p-6 border border-slate-200 hover:shadow-lg transition-all group">
                ...
                <div className="bg-white rounded-xl p-4 border border-slate-100 flex items-center gap-4 min-h-[80px]">
                  ...
                  {(playingIndex === idx && !AUDIO_URLS[idx]) ? (
