    ...
    if (playingIndex === index) {
      ...
      if (audioUrl) {
        const tryPlay = (url: string, fallbacks: string[]) => {
          console.log(`LiveDemo attempting to play audio: ${url}`);
          ...
          const handleError = (e: any) => {
            console.warn(`LiveDemo warning loading/playing audio from ${url}:`, e);
            cleanup();
            if (fallbacks.length > 0) {
              const nextUrl = fallbacks[0];
              const remainingFallbacks = fallbacks.slice(1);
              console.log(`LiveDemo falling back to: ${nextUrl}`);
          ...
          audio.play().catch(err => {
            console.warn(`LiveDemo play promise rejected for ${url}:`, err);
            if (audioRef.current === audio) {
              handleError(err);
            }
          });
