      ...
      `}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between relative h-10">
            ...
            {/* Logo */}
            <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 md:relative md:left-0 md:top-0 md:transform-none md:translate-x-0 md:translate-y-0 flex items-center gap-1 z-10 cursor-pointer" onClick={() => handleNavClick('home')}>
            ...
            {/* Desktop Menu */}
            <div className="hidden md:flex items-center gap-1 absolute left-1/2 transform -translate-x-1/2 h-full">
              ...
              <div className="relative group h-full flex items-center">
                <button className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-slate-600 group-hover:text-primary transition-colors focus:outline-none">
                  Lösungen <ChevronDown size={14} className="group-hover:rotate-180 transition-transform duration-200" />
                </button>
                
                {/* Dropdown Content */}
                <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 w-[800px] hidden group-hover:block transition-all opacity-0 group-hover:opacity-100 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {doctorSpecialties.map((doc, idx) => (
                        <button key={idx} onClick={() => handleNavClick(doc.id)} className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors group/item text-left w-full">
                          <div className="w-10 h-10 shrink-0 rounded-full bg-primary-light/50 flex items-center justify-center text-primary-dark group-hover/item:bg-primary group-hover/item:text-white transition-colors">
                            <doc.icon size={18} />
                          </div>
                          <div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 bg-white pt-24 px-6 animate-in fade-in slide-in-from-top-5 duration-200 overflow-y-auto pb-10">
          <div className="flex flex-col gap-6 text-center pb-10">
              ...
              <div className="grid grid-cols-1 gap-3">
                 {doctorSpecialties.map((doc, idx) => (
                    <button key={idx} onClick={() => handleNavClick(doc.id)} className="flex items-center gap-3 text-slate-700 font-medium text-left w-full">
                      <doc.icon size={16} className="text-primary shrink-0" /> {doc.name}
                    </button>
                 ))}
              </div>
            </div>
            ...
            <button onClick={() => handleNavClick('pricing')} className="text-2xl font-semibold text-slate-800">Preise & Ersparnis</button>
